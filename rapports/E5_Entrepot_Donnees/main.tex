...
\usepackage{placeins} % \FloatBarrier pour éviter que les tableaux/figures se "baladent"
\usepackage{float}    % option [H] et \floatplacement
\usepackage{etoolbox} % hooks (AtBegin/AtEndEnvironment)

% Forcer les flottants à rester "ici" (évite l’empilement de titres quand LaTeX déplace les tables)
\floatplacement{table}{H}
\floatplacement{figure}{H}

% Ajoute automatiquement un \FloatBarrier après chaque figure
...
\hypersetup{
    ...
    urlcolor=azure,
    citecolor=darkblue,
    pdftitle={E5 - Entrepôt de Données},
    pdfauthor={Votre Nom}
}
...
\fancyhead[R]{E5 - Entrepôt de Données}
\fancyfoot[C]{\thepage}
...
L'infrastructure technique repose sur un pipeline ETL développé en \textbf{Python} qui transforme les données sources stockées localement (fichiers CSV et JSON) et les charge dans une base \textbf{Azure SQL Database} structurée en entrepôt de données optimisé pour les requêtes analytiques.
...
Les données proviennent principalement de l'\textbf{INSEE} (Institut National de la Statistique et des Études Économiques) et de l'\textbf{API Géo} du gouvernement français. Les fichiers sources sont stockés localement et comprennent \textbf{11 fichiers CSV} totalisant \textbf{15 394 enregistrements} et \textbf{1 fichier JSON} avec \textbf{3 782 communes}.
...
\midrule
\endhead
population hauts\_de\_france.csv & Population par PCS (Professions et Catégories Socioprofessionnelles), sexe et tranche d'âge. Années : 2010, 2015, 2021 & 1 579 \\
naissances hauts\_de\_france.csv & Naissances vivantes annuelles par département (code EC\_MEASURE=LVB). Période : 2014-2023 & 51 \\
DECES hauts\_de\_france.csv & Décès annuels par département (code EC\_MEASURE=DTH). Période : 2014-2023 & 51 \\
FECONDITE hauts\_de\_france.csv & Indicateurs de fécondité et structure familiale (nombre d'enfants, type de famille). Années : 2010, 2015, 2021 & 321 \\
\bottomrule
...
\midrule
\endhead
CREATION\_ENT hauts\_de\_france.csv & Créations d'entreprises par secteur d'activité (NAF) et forme juridique. Mesures : BURE (entreprises), UNIT\_LOC\_BURE (établissements). Période : 2012-2024 & 10 757 \\
CREA\_EI hauts\_de\_france.csv & Créations d'entrepreneurs individuels par sexe, âge et forme juridique (MICRO, ENTIND). Période : 2012-2024 & 1 561 \\
EMPLOI\_CHOMAGE hauts\_de\_france.csv & Population active par statut d'emploi (EMPSTA\_ENQ : 1=emploi, 2=chômage) et PCS. Tranche d'âge : 15-64 ans & 346 \\
\bottomrule
...
\midrule
\endhead
DS\_FILOSOFI hauts\_de\_france.csv & Indicateurs FILOSOFI : revenus (D1, D9, médiane), taux de pauvreté (PR\_MD60), structure des revenus. Année : 2021 & 101 \\
FILOSOFI\_AGE TP\_NIVVIE.csv & FILOSOFI ventilé par tranche d'âge du référent fiscal. Indicateurs : revenu médian, taux de pauvreté & 71 \\
Logement hauts\_de\_france.csv & Résidences principales par statut de surpeuplement (OVEROCC : 0=normal, 1=surpeuplé). Années : 2010, 2015, 2021 & 46 \\
Menage hauts\_de\_france.csv & Structure des ménages par type (TPH), PCS du référent et taille. Mesures : DWELLINGS, DWELLINGS\_POPSIZE & 511 \\
\bottomrule
...
Un \textbf{entrepôt de données (Data Warehouse)} est un système d'information décisionnel conçu pour
centraliser, historiser et structurer des données issues de sources hétérogènes afin de faciliter
leur analyse et leur exploitation à des fins d'aide à la décision. Contrairement aux bases de données
transactionnelles (OLTP), un Data Warehouse est orienté \textbf{analyse} (OLAP), avec des données
intégrées, historisées, non volatiles et organisées selon des axes métier.

Dans ce projet, l'entrepôt de données constitue le socle analytique permettant l'étude
des dynamiques territoriales (démographie, emploi, revenus, logement, activité économique) à partir
de jeux de données statistiques structurés.
\subsection{Approche de modélisation}
Deux grandes approches méthodologiques existent pour la conception d'un Data Warehouse :

\begin{itemize}
    \item l'approche \textbf{Top-Down} (descendante), popularisée par Inmon
    \item l'approche \textbf{Bottom-Up} (ascendante), proposée par Kimball
\end{itemize}

L'approche \textbf{Top-Down} consiste à concevoir en premier lieu un entrepôt de données global et
normalisé (souvent en 3NF), à partir duquel sont ensuite dérivés des datamarts métiers. Cette approche
offre une vision d'entreprise cohérente mais nécessite un cadrage fonctionnel lourd et des délais de
...
La modélisation logique d'un Data Warehouse peut s'appuyer sur plusieurs types de schémas
dimensionnels, dont l'objectif est d'organiser les données de manière optimale pour l'analyse
décisionnelle. Les trois schémas les plus couramment utilisés sont :

\begin{itemize}
    \item le \textbf{schéma en étoile} (Star Schema),
    \item le \textbf{schéma en flocon} (Snowflake Schema),
    \item le \textbf{schéma en constellation}.
\end{itemize}

Le \textbf{schéma en étoile} est le modèle dimensionnel le plus simple et le plus répandu. Il repose
sur une table de faits centrale contenant les mesures quantitatives (ex. population, revenus,
taux de chômage), reliée directement à des tables de dimensions dénormalisées (temps, géographie,
démographie, activité, logement). Par exemple, une table de faits \texttt{FAIT\_EMPLOI} peut être
liée directement aux dimensions \texttt{DIM\_TEMPS}, \texttt{DIM\_GEOGRAPHIE} et
\texttt{DIM\_DEMOGRAPHIE}. Cette structure limite le nombre de jointures nécessaires, ce qui rend
les requêtes SQL plus lisibles et plus performantes, notamment dans les outils de reporting et de
visualisation.

Le \textbf{schéma en flocon} constitue une évolution plus normalisée du schéma en étoile. Les
dimensions y sont décomposées en plusieurs tables hiérarchiques. Par exemple, la dimension
géographique peut être séparée en tables \texttt{COMMUNE}, \texttt{DEPARTEMENT} et
\texttt{REGION}. Bien que cette approche réduise la redondance des données, elle augmente le nombre
de jointures nécessaires pour les analyses, ce qui complexifie les requêtes et peut impacter les
performances. Ce type de schéma est généralement réservé aux contextes où la normalisation des
référentiels est un enjeu majeur.

Le \textbf{schéma en constellation} (ou galaxie) généralise le principe du schéma en étoile en
introduisant plusieurs tables de faits partageant des dimensions communes. Par exemple, les tables
\texttt{FAIT\_POPULATION}, \texttt{FAIT\_EMPLOI} et \texttt{FAIT\_REVENUS} peuvent partager les
dimensions \texttt{DIM\_TEMPS} et \texttt{DIM\_GEOGRAPHIE}. Cette approche permet d'analyser plusieurs
processus métier de manière cohérente, mais nécessite une gouvernance stricte des dimensions afin
d'assurer la cohérence analytique entre les différents domaines.

Dans le cadre de ce projet, le \textbf{schéma en étoile} a été retenu en raison de sa simplicité de
mise en œuvre, de ses performances élevées en lecture et de sa forte adéquation avec les usages
décisionnels. Il permet une compréhension rapide du modèle par les utilisateurs et facilite la
construction de datamarts analytiques évolutifs.
...
\begin{tikzpicture}[
    dimension/.style={
        rectangle, draw=azure, fill=azure!20,
        minimum width=3.8cm, minimum height=1.2cm,
        text centered, font=\scriptsize
    },
    fact/.style={
        rectangle, draw=darkblue, fill=darkblue!20,
...
% =====================
% Dimensions (gauche, espace réduit)
% =====================
\node[dimension] (temps)  at (-7, 4.5) {DIM\_TEMPS};
\node[dimension] (geo)    at (-7, 3.0) {DIM\_GEOGRAPHIE};
\node[dimension] (demo)   at (-7, 1.0) {DIM\_DEMOGRAPHIE};
\node[dimension] (ind)    at (-7,-0.8) {DIM\_INDICATEUR};
\node[dimension] (act)    at (-7,-2.6) {DIM\_ACTIVITE};
\node[dimension] (logdim) at (-7,-4.4) {DIM\_LOGEMENT};

% =====================
% Tables de faits (droite, plus proches)
% =====================
\node[fact] (pop) at (4, 3.6) {FAIT\_POPULATION};
\node[fact] (evt) at (4, 2.4) {FAIT\_EVENEMENTS};
\node[fact] (ent) at (4, 1.2) {FAIT\_ENTREPRISES};
\node[fact] (emp) at (4, 0.0) {FAIT\_EMPLOI};
\node[fact] (rev) at (4,-1.2) {FAIT\_REVENUS};
\node[fact] (log) at (4,-2.4) {FAIT\_LOGEMENT};

% =====================
% Connexions DROITES et DISTINCTES
% =====================
% FAIT_POPULATION
\draw[arrow] (temps.east) -- (pop.west);
\draw[arrow] (geo.east)   -- ++(0.6,0) -- (pop.west);

% FAIT_EVENEMENTS
\draw[arrow] (temps.east) -- ++(0.6,0) -- (evt.west);
\draw[arrow] (ind.east)   -- (evt.west);

% FAIT_ENTREPRISES
\draw[arrow] (temps.east) -- ++(0.6,0) -- (ent.west);
\draw[arrow] (act.east)   -- (ent.west);

% FAIT_EMPLOI
\draw[arrow] (temps.east) -- ++(0.6,0) -- (emp.west);
\draw[arrow] (demo.east)  -- (emp.west);

% FAIT_REVENUS
\draw[arrow] (temps.east) -- ++(0.6,0) -- (rev.west);
\draw[arrow] (demo.east)  -- ++(0.6,0) -- (rev.west);

% FAIT_LOGEMENT
\draw[arrow] (temps.east)  -- ++(0.6,0) -- (log.west);
\draw[arrow] (logdim.east) -- (log.west);

\end{tikzpicture}
\caption{Architecture logique du Data Warehouse – Espacement réduit entre dimensions et faits}
\end{figure}
\FloatBarrier
...
\centering
\small
\begin{tabular}{p{3.6cm}p{2cm}p{6cm}}
\toprule
...
%\begin{tabular}{>{\raggedright\arraybackslash}p{4.5cm} >{\raggedright\arraybackslash}p{2.4cm} p{6.4cm}}
\toprule
...
\subsubsection{Scripts DDL (Data Definition Language) - Dimensions}
...
\begin{tabular}{lp{8cm}}
\toprule
\textbf{Script SQL} & \textbf{Description} \\
\midrule
001\_create\_schemas.sql & Création des schémas stg, dwh, dm, analytics \\
002\_create\_dimensions.sql & Création des 6 tables de dimensions \\
003\_create\_facts.sql & Création des 6 tables de faits avec FK \\
004\_populate\_dimensions.sql & Alimentation des référentiels (temps, geo, PCS, NAF) \\
005\_create\_datamarts.sql & Création des vues datamarts et analytiques \\
\bottomrule
\end{tabular}
\caption{Scripts SQL de déploiement du Data Warehouse}
\end{table}
\FloatBarrier
...
La connexion repose sur une architecture à trois couches logicielles qui s'enchaînent entre le code Python et la base Azure SQL Database :

\begin{figure}[h]
\centering
\begin{tikzpicture}[
    layer/.style={rectangle, draw=azure, fill=azure!10, minimum width=4cm, minimum height=1.2cm, text centered, font=\small},
    arrow/.style={->, thick, >=stealth}
]
\node[layer] (python) at (0,0) {Code Python\\(scripts ETL)};
\node[layer] (pyodbc) at (5.5,0) {pyodbc\\(bibliothèque Python)};
\node[layer] (odbc) at (11,0) {ODBC Driver 18\\(pilote Microsoft)};
\node[layer, fill=darkblue!15, draw=darkblue] (azure) at (11,-2.5) {Azure SQL Database\\(Data Warehouse)};

\draw[arrow] (python) -- (pyodbc);
\draw[arrow] (pyodbc) -- (odbc);
\draw[arrow] (odbc) -- (azure);
\end{tikzpicture}
\caption{Architecture de connexion : chaîne Python vers Azure SQL}
\end{figure}
\FloatBarrier

\textbf{1. ODBC Driver 18 for SQL Server} \textit{(couche système)}

ODBC (\textit{Open Database Connectivity}) est un standard universel d'accès aux bases de données. Le \textbf{Driver 18} est le pilote fourni par Microsoft, installé directement sur la machine (hors Python). Il assure :
\begin{itemize}[leftmargin=*]
    \item Le \textbf{transport réseau} des requêtes SQL vers le serveur Azure via le protocole TDS (\textit{Tabular Data Stream}) sur le port 1433
    \item Le \textbf{chiffrement TLS} de la connexion (\texttt{Encrypt=yes})
    \item L'\textbf{authentification} auprès du serveur SQL (login/mot de passe)
\end{itemize}

Sans ce driver installé sur la machine, aucune connexion Python vers Azure SQL n'est possible.

\textbf{2. pyodbc} \textit{(couche Python)}

\texttt{pyodbc} est une bibliothèque Python (installée via \texttt{pip install pyodbc}) qui sert de pont entre le code Python et le driver ODBC. Elle permet d'ouvrir des connexions, d'exécuter des requêtes SQL et de récupérer les résultats directement en Python.

Dans le projet, la connexion est construite ainsi (extrait de \texttt{deploy\_dwh.py}) :

\begin{lstlisting}[language=Python, caption=Construction de la connexion pyodbc]
import pyodbc

conn_str = (
    "DRIVER={ODBC Driver 18 for SQL Server};"  # Pilote a utiliser
    "SERVER=sqlelbrek-prod.database.windows.net,1433;"  # Serveur Azure + port
    "DATABASE=projet_data_eng;"                # Nom de la base
    "UID=sqladmin;"                            # Login administrateur
    "PWD=<mot_de_passe>;"                      # Mot de passe
    "Encrypt=yes;"                             # Chiffrement TLS active
    "TrustServerCertificate=yes;"              # Accepter le certificat
)

conn = pyodbc.connect(conn_str, autocommit=True)
cursor = conn.cursor()
cursor.execute("SELECT * FROM dwh.dim_temps")
\end{lstlisting}

\textbf{3. SQLAlchemy} \textit{(couche abstraction -- optionnelle)}

SQLAlchemy est un ORM (\textit{Object-Relational Mapper}) Python de plus haut niveau. Il utilise pyodbc en interne mais offre une interface plus pratique, notamment pour charger des \texttt{DataFrame} pandas directement dans SQL. La chaîne de connexion SQLAlchemy suit un format URL :

\begin{lstlisting}[caption=Format de connexion SQLAlchemy]
mssql+pyodbc://sqladmin:<pwd>@sqlelbrek-prod.database.windows.net:1433/projet_data_eng?driver=ODBC+Driver+18+for+SQL+Server
\end{lstlisting}
...
\toprule
\textbf{Segment} & \textbf{Signification} \\
\midrule
\texttt{mssql} & Type de base de données (Microsoft SQL Server) \\
\texttt{+pyodbc} & Connecteur Python à utiliser (pyodbc) \\
\texttt{sqladmin:<pwd>} & Identifiants de connexion (login et mot de passe) \\
\texttt{@sqlelbrek-prod...} & Adresse du serveur Azure SQL \\
\texttt{:1433} & Port réseau (port standard SQL Server) \\
\texttt{/projet\_data\_eng} & Nom de la base de données cible \\
\texttt{?driver=ODBC+Driver+18...} & Pilote ODBC à utiliser sur la machine \\
\bottomrule
\end{tabular}
\caption{Décomposition de la chaîne de connexion SQLAlchemy}
\end{table}
\FloatBarrier
...
\toprule
\textbf{Composant} & \textbf{Type} & \textbf{Rôle} \\
\midrule
ODBC Driver 18 & Logiciel système & Pilote réseau installé sur la machine, gère le transport et le chiffrement \\
pyodbc & Bibliothèque Python & Pont entre Python et le driver ODBC, exécute les requêtes SQL \\
SQLAlchemy & Bibliothèque Python & Couche d'abstraction haut niveau, facilite le chargement de données via pandas \\
\bottomrule
\end{tabular}
\caption{Récapitulatif des trois composants de connexion}
\end{table}
\FloatBarrier
...
\node[step] (s2) at (5.2,0) {2. Dimensions\\(Référentiels)};
\node[step] (s3) at (9.8,0) {3. Faits\\(Mesures)};
\node[step] (s4) at (14.0,0) {4. Refresh\\(Stats/Vues)};
...
\midrule
1 & export\_to\_sql.py & Charge les CSV locaux vers les tables de staging (dbo.stg\_*) \\
...
\midrule
Cloud & Microsoft Azure & Hébergement de la base de données \\
Base de données & Azure SQL Database & Entrepôt de données \\
IaC & Terraform & Infrastructure as Code \\
ETL & Python + pandas + SQLAlchemy & Scripts de transformation et chargement \\
Stockage source & Fichiers locaux (CSV, JSON) & Données sources INSEE \\
...
\begin{enumerate}[leftmargin=*]
    ...
    \begin{itemize}
        \item Déploiement reproductible et versionné
        \item Automatisation complète du provisioning Azure SQL Database
        \item Facilité de recréation de l'environnement
    \end{itemize}

    \item \textbf{Pipeline ETL local simple et maîtrisé}
    \begin{itemize}
        \item Scripts Python exécutés localement, facilement débogables
        \item Pas de dépendance à des services cloud supplémentaires
        \item Contrôle total sur le processus de transformation
    \end{itemize}
    ...
    \item \textbf{Séparation des schémas}
    \begin{itemize}
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Modélisation} : Schéma en étoile avec 6 dimensions et 6 tables de faits, couvrant les domaines démographie, économie, social et logement
    \item \textbf{Infrastructure} : Déploiement automatisé sur Azure SQL Database via Terraform
    \item \textbf{ETL} : Pipeline Python local complet pour l'alimentation des dimensions et faits à partir des fichiers locaux
    \item \textbf{Datamarts} : 5 vues agrégées par domaine métier + tableau de bord territorial
...
L'entrepôt de données constitue une base solide pour de futures évolutions. Plusieurs axes d'amélioration ont été identifiés pour un projet ultérieur :

\begin{itemize}
    \item \textbf{Stockage centralisé} : Migrer les fichiers sources vers un \textbf{Azure Data Lake Storage (ADLS Gen2)} pour centraliser les données brutes dans le cloud et permettre un accès distribué
    \item \textbf{Orchestration cloud} : Utiliser \textbf{Azure Data Factory} pour orchestrer les pipelines ETL au lieu de les exécuter localement, permettant une planification automatisée, un monitoring intégré et une meilleure fiabilité
    \item \textbf{Traitement distribué} : Intégrer \textbf{Azure Databricks} (Spark) pour les traitements de données à grande échelle et les transformations complexes
    \item \textbf{Sécurité des secrets} : Utiliser \textbf{Azure Key Vault} pour la gestion sécurisée des credentials au lieu de variables d'environnement
    \item \textbf{Visualisation} : Développer des tableaux de bord \textbf{Power BI} connectés aux datamarts
    \item \textbf{Nouvelles sources} : Intégrer de nouvelles sources de données (SIRENE, DVF, etc.)
    \item \textbf{Machine Learning} : Mettre en place des analyses prédictives sur les tendances territoriales
    \item \textbf{Extension géographique} : Étendre l'analyse à d'autres régions françaises
\end{itemize}

Ces points d'amélioration feront l'objet d'un projet et d'un rapport séparés.
